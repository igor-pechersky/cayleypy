...
class BFSBenchmark:
    ...
    def run_single_benchmark(
        self, bfs_func, algorithm_name: str, graph: CayleyGraph, graph_name: str, max_diameter: int
    ) -> BenchmarkResult:
        """Run a single benchmark and return results."""
        self.logger.info("Benchmarking %s on %s (max_diameter=%d)", algorithm_name, graph_name, max_diameter)
        ...
        try:
            ...
            self.logger.info(
                "✓ %s completed: %.3fs, %.2f MB increase, growth: %s",
                algorithm_name,
                execution_time,
                memory_increase,
                growth_function,
    ...
    def compare_algorithms(self, reference_result: BenchmarkResult, test_result: BenchmarkResult) -> ComparisonResult:
        """Compare two algorithm results."""
        # Check numeric correctness
        numeric_identical = (
            reference_result.success
            and test_result.success
            and reference_result.growth_function == test_result.growth_function
        )
    ...
    def run_comprehensive_benchmark(self) -> Dict[str, Any]:
        ...
        for graph, graph_name, max_diameter in benchmark_graphs:
            ...
            for bfs_func, algorithm_name in algorithms:
                result = self.run_single_benchmark(bfs_func, algorithm_name, graph, graph_name, max_diameter)
            ...
            if graph.definition.state_size > 8 and graph.definition.is_permutation_group():
                ...
                # CPU bitmask
                result = self.run_single_benchmark(bfs_bitmask, "CPU Bitmask BFS", graph, graph_name, max_diameter)
            ...
            if "NumPy BFS" in graph_results and "TPU BFS" in graph_results:
                comparison = self.compare_algorithms(graph_results["NumPy BFS"], graph_results["TPU BFS"])
                self.logger.info(
                    "TPU BFS vs NumPy BFS: %.2fx speedup, %.2fx memory, identical=%s",
                    comparison.speedup,
                    comparison.memory_ratio,
                    comparison.numeric_identical,
                )

            if "CPU Bitmask BFS" in graph_results and "TPU Bitmask BFS" in graph_results:
                comparison = self.compare_algorithms(graph_results["CPU Bitmask BFS"], graph_results["TPU Bitmask BFS"])
                self.logger.info(
                    "TPU Bitmask vs CPU Bitmask: %.2fx speedup, %.2fx memory, identical=%s",
                    comparison.speedup,
                    comparison.memory_ratio,
                    comparison.numeric_identical,
                )
    ...
    def print_summary_report(self):
        ...
        for comparison in self.comparisons:
            ...
            print(
                f"  {status} {comparison.test_algorithm} vs {comparison.reference_algorithm} "
                f"on {comparison.graph_name}:"
            )
            print(
                f"      Speedup: {comparison.speedup:.2f}x, Memory: {comparison.memory_ratio:.2f}x, "
                f"Correct: {comparison.numeric_identical}"
            )
