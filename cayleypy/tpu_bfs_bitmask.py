...
@jax.jit
def _materialize_permutations_jit(
    black: jnp.ndarray, map1: jnp.ndarray, encoded_suffix: jnp.int64
) -> jnp.ndarray:
...
class TPUVertexChunk(nnx.Module):
    ...
    def __init__(self, n: int, suffix: List[int], backend: TPUBackend, rngs: nnx.Rngs):
        ...
        self.encoded_suffix = nnx.Variable(
            jnp.int64(sum(suffix[i - R] << (4 * i) for i in range(R, n)))
        )
    ...
    def materialize_last_layer_permutations(self) -> jnp.ndarray:
        ...
        perms = _materialize_permutations_jit(
            self.last_layer.value, self.map1.value, self.encoded_suffix.value
        )
...
class TPUBitmaskBFSModule(nnx.Module):
    ...
    def __init__(self, graph, backend: TPUBackend, rngs: nnx.Rngs):
        ...
        self.logger.info(
            ...
            n, len(self.chunks), len(graph_def.generators_permutations)
        )
    ...
    def run_bfs(self, max_diameter: int = 10**6) -> List[int]:
        ...
        self.bfs_state.value.update({
            ...
            'diameter': 0,
            'total_states_found': layer_sizes[0],
            'is_complete': False
        })
        ...
        for diameter in range(1, max_diameter + 1):
            ...
            self.logger.debug("Diameter %d: %d new states (total: %d)", 
                            diameter, layer_size, self.bfs_state.value['total_states_found'])
        
        final_diameter = self.bfs_state.value['diameter']
        total_states = self.bfs_state.value['total_states_found']
        
        self.logger.info("TPU bitmask BFS completed: diameter=%d, total_states=%d", 
    ...
    def verify_int64_precision(self) -> bool:
        ...
        try:
            ...
            self.logger.info("int64 precision test: dtype=%s, large_value=%s", 
                           encoded.dtype, is_large)
...
def create_tpu_bitmask_bfs(graph, backend: Optional[TPUBackend] = None) -> TPUBitmaskBFSModule:
    ...
    if not JAX_AVAILABLE:
        raise ImportError(
            "JAX and Flax are required for TPU bitmask BFS. " +
