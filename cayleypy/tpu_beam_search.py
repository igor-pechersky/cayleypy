...
@jax.jit
def _expand_beam_jit(beam_states: jnp.ndarray, generators: jnp.ndarray, _beam_size: int) -> jnp.ndarray:
...
class TPUBeamSearchModule(nnx.Module):
    ...
    def __init__(
        self, graph, predictor_module, backend: TPUBackend, rngs: nnx.Rngs, beam_size: int = 100, max_depth: int = 100
    ):
    ...
    def search_step(
        self, current_beam: jnp.ndarray, target_state: Optional[jnp.ndarray] = None
    ...
    def search(
        self, target_state: Optional[jnp.ndarray] = None, start_state: Optional[jnp.ndarray] = None
    ) -> Dict[str, Any]:
        """Run complete beam search leveraging TPU v6e's memory and compute."""
        self.initialize_search(start_state)

        self.logger.info("Starting TPU beam search: beam_size=%d, max_depth=%d", self.beam_size, self.max_depth)
        ...
        for depth in range(self.max_depth):
            ...
            self.logger.debug(
                "Depth %d: beam_size=%d, best_score=%.4f",
                depth + 1,
                len(current_beam),
                self.search_state.value["best_score"],
            )

        final_depth = self.search_state.value["search_depth"]
        total_explored = self.search_state.value["total_states_explored"]

        self.logger.info("Beam search completed: depth=%d, states_explored=%d", final_depth, total_explored)
    ...
    def get_search_result(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "final_beam_size": (
                len(self.search_state.value["current_beam"])
                if self.search_state.value["current_beam"] is not None
                else 0
            ),
            "final_beam_scores": (
                self.search_state.value["current_scores"].tolist()
                if self.search_state.value["current_scores"] is not None
                else []
            ),
        }
    ...
    def get_performance_metrics(self) -> Dict[str, Any]:
        ...
        predictor_stats = (
            self.predictor_module.get_performance_metrics()
            if hasattr(self.predictor_module, "get_performance_metrics")
            else {}
        )
        ...
        return {
            **base_metrics,
            ...
            "search_efficiency": {
                ...
                "deduplication_ratio": base_metrics["deduplication_operations"]
                / max(1, base_metrics["beam_expansions"]),
...
def _fallback_to_cpu_beam_search(
    _graph, _predictor, _beam_size: int, _max_depth: int, _target_state=None, _start_state=None
) -> Dict[str, Any]:
...
def create_tpu_beam_search(
    graph, predictor, backend: Optional[TPUBackend] = None, beam_size: int = 100, max_depth: int = 100
) -> TPUBeamSearchModule:
    """Factory function to create TPU beam search module with error handling."""
    if not JAX_AVAILABLE:
        raise ImportError(
            "JAX and Flax are required for TPU beam search. " + "Install with: pip install 'cayleypy[jax-tpu]'"
        )
...
def benchmark_tpu_vs_cpu_beam_search(
    graph, predictor, beam_size: int = 50, max_depth: int = 20, target_state: Optional[jnp.ndarray] = None
) -> Dict[str, Any]:
    ...
    results = {
        ...
        "search_params": {"beam_size": beam_size, "max_depth": max_depth, "has_target": target_state is not None},
        "tpu_available": False,
        "cpu_time": 0.0,
        "tpu_time": 0.0,
        "speedup": 0.0,
        ...
        try:
            ...
            if backend.is_available:
                ...
                results["results_comparable"] = (
                    cpu_result.get("target_found") == tpu_result.get("target_found")
                    and abs(cpu_result.get("search_depth", 0) - tpu_result.get("search_depth", 0)) <= 2
                )
