...
class BFSValidator:
    ...
    def validate_memory_usage(
        ...
        if test_peak <= ref_peak * tolerance_factor:
            self.logger.info("✓ %s: Memory usage acceptable (%.2f MB vs %.2f MB reference)", 
                           algorithm_name, test_peak, ref_peak)
            return True
        else:
            self.logger.error("✗ %s: Memory usage too high (%.2f MB vs %.2f MB reference, %.1fx)", 
                            algorithm_name, test_peak, ref_peak, test_peak / max(ref_peak, 1))
            return False
    ...
    def validate_performance_improvement(
        ...
        if test_time > 0:
            ...
            if speedup >= min_speedup:
                self.logger.info("✓ %s: Performance improvement %.2fx (%.3fs vs %.3fs)", 
                               algorithm_name, speedup, test_time, reference_time)
                return True
            else:
                self.logger.warning("⚠ %s: Performance improvement only %.2fx (%.3fs vs %.3fs)", 
                                  algorithm_name, speedup, test_time, reference_time)
                return False
        else:
    ...
    def run_bfs_with_profiling(
        ...
        self.logger.info("Starting %s on graph with %d generators", 
                        algorithm_name, len(graph.definition.generators_permutations))
        ...
        self.logger.info("%s completed in %.3fs, growth function: %s", 
                        algorithm_name, elapsed_time, result)
...
class TestTPUBFSValidation:
    ...
    def _create_test_graphs(self) -> List[Tuple[CayleyGraph, str, int]]:
        ...
        graphs.append((
            CayleyGraph(PermutationGroups.symmetric_group(4)),
            "S4 (24 elements)",
            10
        ))
        
        graphs.append((
            CayleyGraph(PermutationGroups.alternating_group(5)),
            "A5 (60 elements)", 
            15
        ))
        ...
        graphs.append((
            CayleyGraph(PermutationGroups.symmetric_group(5)),
            "S5 (120 elements)",
            20
        ))
        
        graphs.append((
            CayleyGraph(PermutationGroups.dihedral_group(12)),
            "D12 (24 elements)",
            15
        ))
        ...
        try:
            ...
            if available_memory_gb > 8:  # At least 8GB available
                graphs.append((
                    CayleyGraph(PermutationGroups.symmetric_group(6)),
                    "S6 (720 elements)",
                    25
                ))
                ...
                if available_memory_gb > 16:  # At least 16GB available
                    graphs.append((
                        CayleyGraph(PermutationGroups.symmetric_group(7)),
                        "S7 (5040 elements)",
                        30
                    ))
        except Exception as e:  # pylint: disable=broad-exception-caught
    ...
    def test_tpu_bitmask_bfs_numeric_correctness(self):
        ...
        bitmask_graphs = [(g, name, max_d) for g, name, max_d in self.test_graphs 
    ...
    def test_tpu_bfs_memory_efficiency(self):
        ...
        if test_graph.definition.state_size > 8:
            implementations.extend([
                (bfs_bitmask, "CPU Bitmask BFS"),
                (tpu_bfs_bitmask, "TPU Bitmask BFS")
            ])
        ...
        for bfs_func, name in implementations:
            try:
                _, _, memory_stats = self.validator.run_bfs_with_profiling(
                    bfs_func, test_graph, max_diameter, name
                )
        ...
        for name, stats in memory_results.items():
            self.validator.logger.info("  %s: %.2f MB peak increase", 
                                     name, stats['peak_increase_mb'])
        ...
        if "NumPy BFS" in memory_results and "TPU BFS" in memory_results:
            ...
            assert tpu_memory <= ref_memory * 3.0, f"TPU BFS uses too much memory: {tpu_memory:.2f} MB vs {ref_memory:.2f} MB"
        ...
        if "CPU Bitmask BFS" in memory_results and "TPU Bitmask BFS" in memory_results:
            ...
            assert tpu_memory <= ref_memory * 2.5, f"TPU Bitmask BFS uses too much memory: {tpu_memory:.2f} MB vs {ref_memory:.2f} MB"
    ...
    def test_tpu_bfs_performance_scaling(self):
        ...
        try:
            ...
            if available_memory_gb > 8:
                scaling_graphs.append((
                    CayleyGraph(PermutationGroups.symmetric_group(6)), "S6", 20
                ))
        ...
        for graph, name, max_diameter in scaling_graphs:
            ...
            self.validator.logger.info("%s (size %d): CPU %.3fs, TPU %.3fs, speedup %.2fx", 
                                     name, graph_size, cpu_time, tpu_time, speedup)
    ...
    def test_tpu_bfs_precision_maintenance(self):
        ...
        test_graph = CayleyGraph(PermutationGroups.symmetric_group(4))
...
def run_comprehensive_validation():
    ...
    try:
        ...
        test_graphs = [
            (CayleyGraph(PermutationGroups.symmetric_group(4)), "S4", 10),
            (CayleyGraph(PermutationGroups.symmetric_group(5)), "S5", 15),
            (CayleyGraph(PermutationGroups.alternating_group(5)), "A5", 15),
        ]
        ...
        for graph, name, max_diameter in test_graphs:
            ...
            # Validation
            numeric_ok = validator.validate_growth_functions_identical(
                ref_result, tpu_result, f"TPU BFS ({name})"
            )
            
            memory_ok = validator.validate_memory_usage(
                ref_memory, tpu_memory, f"TPU BFS ({name})"
            )
            
            perf_ok = validator.validate_performance_improvement(
                ref_time, tpu_time, f"TPU BFS ({name})"
            )
        ...
        bitmask_graphs = [(g, n, d) for g, n, d in test_graphs 
                         if g.definition.state_size > 8 and g.definition.is_permutation_group()]
        ...
        for graph, name, max_diameter in bitmask_graphs:
            ...
            # Validation
            numeric_ok = validator.validate_growth_functions_identical(
                ref_result, tpu_result, f"TPU Bitmask ({name})"
            )
            
            memory_ok = validator.validate_memory_usage(
                ref_memory, tpu_memory, f"TPU Bitmask ({name})"
            )
